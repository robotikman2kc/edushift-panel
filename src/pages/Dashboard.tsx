...
const Dashboard = () => {
  ...
  const fetchCalendarNotes = async () => {
    try {
      ...
      // Get calendar notes for selected month
  ...
  const fetchNextMonthNotes = async () => {
    try {
      ...
      // Get next month based on selected date
      const nextMonth = new Date(currentYear, currentMonth + 1, 1);
      const day7NextMonth = new Date(currentYear, currentMonth + 1, 7);
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <Card>
            ...
            <CardContent>
              ...
              ) : calendarNotes.length === 0 && nextMonthNotes.length === 0 ? (
                <EmptyState
                  icon={CalendarDays}
                ...
                <div className="space-y-3 max-h-[400px] overflow-y-auto">
                        ...
                        {calendarNotes.map((note) => (
                          <div 
                            ...
                            className={`p-2 border rounded-lg transition-all duration-200 cursor-pointer ${
                              note.type === 'holiday' 
                                ? 'bg-red-50 dark:bg-red-950 border-red-200 dark:border-red-800 hover:bg-red-100 dark:hover:bg-red-900 hover:shadow-md hover:scale-[1.02]' 
                                : 'bg-yellow-50 dark:bg-yellow-950 border-yellow-200 dark:border-yellow-800 hover:bg-yellow-100 dark:hover:bg-yellow-900 hover:shadow-md hover:scale-[1.02]'
                            }`}
                            onClick={() => {
                  ...
                  {nextMonthNotes.length > 0 && (
                    <div>
                      ...
                      <h4 className="text-xs font-semibold text-muted-foreground mb-2 px-1">
                        {format(new Date(selectedDate.getFullYear(), selectedDate.getMonth() + 1, 1), "MMMM yyyy", { locale: idLocale })} (1-7)
                      </h4>
                      <div className="space-y-2">
                          ...
                          <div 
                            ...
                            className={`p-2 border rounded-lg transition-all duration-200 cursor-pointer ${
                              note.type === 'holiday' 
                                ? 'bg-red-50 dark:bg-red-950 border-red-200 dark:border-red-800 hover:bg-red-100 dark:hover:bg-red-900 hover:shadow-md hover:scale-[1.02]' 
                                : note.type === 'periode'
                                ? 'bg-blue-50 dark:bg-blue-950 border-blue-200 dark:border-blue-800 hover:bg-blue-100 dark:hover:bg-blue-900 hover:shadow-md hover:scale-[1.02]'
                                : 'bg-yellow-50 dark:bg-yellow-950 border-yellow-200 dark:border-yellow-800 hover:bg-yellow-100 dark:hover:bg-yellow-900 hover:shadow-md hover:scale-[1.02]'
                            }`}
                            onClick={() => {
                              const noteDate = new Date(note.tanggal);
